// =============================================================================
// Browser Video Recorder Library — zip-export.js
...
export async function exportToZip(dirHandle, zip, btnVideo, refreshUI, recorderRef) {
  ...
  if (dirHandle) {
    ...
    if (recorderRef && typeof recorderRef.getExportFilename === "function") {
      zipFilename = recorderRef.getExportFilename("zip");
    } else {
      zipFilename = `Sine-Gordon-Render_${Date.now()}.zip`;
    }
    let saveHandle = null;
    if (isLocal) {
        try {
    ...
    try {
      ...
      for (let i = 0; i < frameFiles.length; i++) {
          ...
          if (percentEl) {
              ...
              if (bottomPhaseEl) bottomPhaseEl.textContent = "Packaging ZIP...";
              if (bottomFramesEl) bottomFramesEl.textContent = `${i + 1} / ${frameFiles.length} frames`;
          }
      }
    ...
    for (let i = 0; i < frameNames.length; i++) {
        ...
        if (percentEl) {
            ...
            const bottomFramesEl = document.getElementById("assembly-bottom-frames");
            if (bottomPhaseEl) bottomPhaseEl.textContent = "Packaging ZIP...";
            if (bottomFramesEl) bottomFramesEl.textContent = `${i + 1} / ${frameNames.length} frames`;
        }
    }
    ...
    zip.generateAsync({ 
      ...
      let memoryZipFilename;
      if (recorderRef && typeof recorderRef.getExportFilename === "function") {
        memoryZipFilename = recorderRef.getExportFilename("zip");
      } else {
        memoryZipFilename = "Sine-Gordon-Render_" + Date.now() + ".zip";
      }
      ...
      if (window.showSaveFilePicker) {
        try {
          const pickerOpts = {
            id: 'zip-export',
            suggestedName: memoryZipFilename,
            types: [{ description: 'ZIP Files', accept: { 'application/zip': ['.zip'] } }],
          };
          ...
          if (e.name === "AbortError") {
            ...
            console.warn("[ZIP Picker] Save file picker failed. Falling back to traditional Blob download.", e);
            window.saveAs(content, memoryZipFilename);
          }
        }
      } else {
        window.saveAs(content, memoryZipFilename);
